...
const DropShadow = styled.div.attrs({
  style: (props) => ({
    boxShadow: `0px ${props.length}px ${props.spread}px 0px rgba(0,0,0, ${props.opacity})`,
...
const IconContainer = styled.div.attrs({
  style: (props) => ({
    transform: `rotateX(${props.rx}deg) rotateY(${props.ry}deg) scale3d(${props.scale}, ${props.scale}, ${props.scale})`,
  }),
})`
  position: relative;
  width: 100%;
...
export default class AppleTVIcon extends PureComponent {
  ...
  render() {
    ...
    const styles = {
      scale: isOver ? (isSelecting ? spring(1.05) : spring(1.1)) : spring(1),
      dx: isOver ? spring(dx) : spring(0),
      dy: isOver ? spring(dy) : spring(0),
      rx: isOver ? (isSelecting ? spring(rx * 1.4) : spring(rx)) : spring(0),
      ry: isOver ? (isSelecting ? spring(ry * 1.4) : spring(ry)) : spring(0),
